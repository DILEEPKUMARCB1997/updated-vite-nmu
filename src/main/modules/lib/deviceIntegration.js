  ...
  try {
    ...
    Object.keys(deviceGroupList).forEach((groupId) => {
      if (groupId !== 'unGrouped') {
        Object.keys(deviceGroupList[groupId].deviceList).forEach((MACAddress) => {
          if (groupId !== 'unGrouped' && deviceOnlineList[MACAddress] === undefined) {
            deviceOnlineList[MACAddress] = {
              model: deviceGroupList[groupId].deviceList[MACAddress].model,
              IPAddress: deviceGroupList[groupId].deviceList[MACAddress].IPAddress,
              MACAddress: deviceGroupList[groupId].deviceList[MACAddress].MACAddress,
              deviceType: deviceGroupList[groupId].deviceList[MACAddress].deviceType,
  ...
  try {
    ...
    Object.values(onlineDeviceList).forEach((element) => {
      ...
      if (element.deviceType !== 'gwd') {
        supportSNMPOnlineMACList = [...supportSNMPOnlineMACList, element.MACAddress]
      }
    })
    ...
    setTimeout(() => {
      Object.keys(deviceGroupList).forEach((groupId) => {
        if (groupId !== 'unGrouped') {
          ...
          Object.keys(deviceGroupList.unGrouped.deviceList).forEach((MACAddress) => {
  ...
  try {
    switch (tmp.cmd) {
      case 'onlineDevice': {
        ...
        if (tmp.isGroup) {
          ...
          Object.keys(deviceGroupList).forEach((groupId) => {
            if (
              groupId !== 'unGrouped' &&
              deviceGroupList[groupId].deviceList[tmp.device.MACAddress] !== undefined
            ) {
              // check device type
              if (
                deviceGroupList[groupId].deviceList[tmp.device.MACAddress].deviceType === 'gwd' &&
                tmp.deviceType === 'snmp'
              ) {
                dt = 'all'
              } else if (
                deviceGroupList[groupId].deviceList[tmp.device.MACAddress].deviceType === 'snmp' &&
                tmp.deviceType === 'gwd'
              ) {
                dt = 'all'
              } else if (
                deviceGroupList[groupId].deviceList[tmp.device.MACAddress].deviceType === 'all'
              ) {
                dt = 'all'
              }

              // if device is offline
              if (!deviceGroupList[groupId].deviceList[tmp.device.MACAddress].online) {
                ...
                if (!tmpOnlineStatus[tmp.device.MACAddress].online) {
                  ...
                  // show notification
                  notificationManagement.default.showDeviceOnline(true, tmp.device)
              ...
              if (tmp.deviceType === 'gwd' && dt === 'all') {
                deviceGroupList[groupId].deviceList[tmp.device.MACAddress].deviceType = dt
              } else {
          ...
          if (deviceGroupList.unGrouped.deviceList[tmp.device.MACAddress] !== undefined) {
            if (
              deviceGroupList.unGrouped.deviceList[tmp.device.MACAddress].deviceType === 'gwd' &&
              tmp.deviceType === 'snmp'
            ) {
              dt = 'all'
            } else if (
              deviceGroupList.unGrouped.deviceList[tmp.device.MACAddress].deviceType === 'snmp' &&
              tmp.deviceType === 'gwd'
            ) {
              dt = 'all'
            } else if (
              deviceGroupList.unGrouped.deviceList[tmp.device.MACAddress].deviceType === 'all'
            ) {
              dt = 'all'
            }
          }
          if (tmp.deviceType === 'gwd' && dt === 'all') {
            deviceGroupList.unGrouped.deviceList[tmp.device.MACAddress].deviceType = dt
          } else {
      ...
      case 'offlineDevice': {
        ...
        Object.keys(deviceGroupList).forEach((groupId) => {
          if (groupId !== 'unGrouped') {
            if (deviceGroupList[groupId].deviceList[tmp.MACAddress] !== undefined) {
              if (tmp.hasDisable.includes('gwd')) {
                if (deviceGroupList[groupId].deviceList[tmp.MACAddress].deviceType === 'all') {
                  deviceGroupList[groupId].deviceList[tmp.MACAddress].deviceType = 'snmp'
                } else if (
                  deviceGroupList[groupId].deviceList[tmp.MACAddress].deviceType === 'gwd'
                ) {
                  deviceGroupList[groupId].deviceList[tmp.MACAddress].deviceType = 'unknown'
                  deviceGroupList[groupId].deviceList[tmp.MACAddress].online = false
                  hasOffline = true
                }
              }
      ...
      case 'deleteGroup': {
        ...
        Object.keys(deviceGroupList[tmp.groupId].deviceList).forEach((MACAddress) => {
          isGroup = false
          Object.keys(deviceGroupListCopy).every((key) => {
            Object.keys(deviceGroupListCopy[key].deviceList).every((key2) => {
              if (deviceGroupListCopy[key].deviceList[key2].MACAddress === MACAddress) {
                isGroup = true
                return false
              }
              return true
          ...
          if (!isGroup && deviceGroupList[tmp.groupId].deviceList[MACAddress].online) {
            deviceGroupList.unGrouped.deviceList[MACAddress] = {
              model: deviceGroupList[tmp.groupId].deviceList[MACAddress].model,
              IPAddress: deviceGroupList[tmp.groupId].deviceList[MACAddress].IPAddress,
              MACAddress: deviceGroupList[tmp.groupId].deviceList[MACAddress].MACAddress,
              netmask: deviceGroupList[tmp.groupId].deviceList[MACAddress].netmask,
              gateway: deviceGroupList[tmp.groupId].deviceList[MACAddress].gateway,
              hostname: deviceGroupList[tmp.groupId].deviceList[MACAddress].hostname,
              kernel: deviceGroupList[tmp.groupId].deviceList[MACAddress].kernel,
              ap: deviceGroupList[tmp.groupId].deviceList[MACAddress].ap,
              online: deviceGroupList[tmp.groupId].deviceList[MACAddress].online,
              isDHCP: deviceGroupList[tmp.groupId].deviceList[MACAddress].isDHCP,
              deviceType: deviceGroupList[tmp.groupId].deviceList[MACAddress].deviceType
            }
          }
        })
      ...
      case 'addRemoveDevice': {
        ...
        Object.keys(tmp.modifyGroupList.addDeviceList).forEach((MACAddress) => {
          deviceGroupList[tmp.groupId].deviceList[MACAddress] = {
            ...
            MACAddress: tmp.modifyGroupList.addDeviceList[MACAddress].MACAddress,
            ...
            deviceType: tmp.modifyGroupList.addDeviceList[MACAddress].deviceType
          }
        ...
        tmp.modifyGroupList.removeDeviceList.forEach((element) => {
          const modelInfoCopy = _.cloneDeep(deviceGroupList[tmp.groupId].deviceList[element])
    ...
    async.series(
      {
        gwdDiscovery: (next) => {
          ...
          const requests = Object.keys(tmpDeviceList).map(
            (MACAddress) => new Promise((resolve) => gwdOnlineStatus(MACAddress, resolve))
          )
          gwd.default.initialize()
        ...
        checkOnlineStatus: (next) => {
          ...
          setTimeout(() => {
              ...
              if (groupId !== 'unGrouped') {
                Object.keys(deviceGroupList[groupId].deviceList).forEach((MACAddress) => {
                  if (!deviceGroupList[groupId].deviceList[MACAddress].online) {
                    const { model, IPAddress } = deviceGroupList[groupId].deviceList[MACAddress]
            ...
            Object.keys(offlineDeviceList).forEach((MACAddress) => {
              if (tmpOnlineStatus[MACAddress] !== undefined) {
                if (!tmpDeviceList[MACAddress].online && tmpOnlineStatus[MACAddress].online) {

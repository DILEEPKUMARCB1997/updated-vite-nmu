...
import { groupManagement, deviceIntegration, telegramManagement, eventLogManagement } from '..'
...
function checkPortUpandDownTask() {
  ...
  return async.forever(
    (next) => {
      ...
      snmpSession = snmp.createSession(deviceList.IPAddress, 'public', snmpOptions)
      ...
      snmpSession.subtree(
        ...
        (varbinds) => {
          try {
            ...
            if (!firstRun) {
              diffPortDetails = [...comparePortDetails(prevPortUpDetails, currentPortUpDetails)]
            }

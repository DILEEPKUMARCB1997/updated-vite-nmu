...
function getDeviceScanByMAC(MacAddress) {
  try {
    const result = apiCore.db.getDeviceScanByMAC({ MACAddress: MacAddress }, true)
...
ipcMain.on(REQUEST_MP_SET_SNMP_SETTINGS, (event, arg) => {
  ...
  try {
    ...
    if (arg.SNMPPollInterval === undefined || Number.isNaN(arg.SNMPPollInterval)) {
      event.sender.send(eventName, {
        success: false,
    ...
    if (arg.version === undefined || (arg.version !== 'v2c' && arg.version !== 'v1')) {
      event.sender.send(eventName, {
        success: false,
    ...
    if (success === true) {
      ...
      if (snmpOldSettings.data.SNMPPollInterval !== arg.SNMPPollInterval || arg.isEnable) {

...
function scheduleBackup() {
  ...
  if (!scheduledData.success) {
    ...
    schedule.forEach((scheduleId) => {
      ...
      Object.keys(scheduledBackup[scheduleId].deviceList).forEach((MACAddress) => {
        devices.push(scheduledBackup[scheduleId].deviceList[MACAddress].MACAddress)
      })
      if (frequency === 1) {
        cron.schedule(nowMinutes + ' ' + nowHours + ' ' + nowDate + ' ' + nowMonth + ' *', () => {
